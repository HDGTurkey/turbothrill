...
const EventDetailPage: React.FC = () => {
     ...
     async function getEvent() {
          ...
          setEventData(await agcContext.executeQueryWhere(eventsModel , 'slug_name' , 'event-detaila')) // 
          console.log(slugName);
          
          console.log("asdfsakfl");
               ...
               <div className='max-w-6xl mx-8 '>
                    <div className='flex text-2xl text-left max-w-6xl mx-auto p-5 '>
                         <div className=' flex-1'>
                              <h1 className="text-3xl font-bold mb-3">{eventData?.name}</h1>
                         </div>
                    </div>
                    <div className={`shadow-lg grid grid-cols-1 lg:grid-cols-2 lg:flex ${themeData.theme === 'light' ? 'bg-gray-100 text-black' : 'bg-gray-800 text-white'}`}>
                         <div className={`w-auto ${themeData.theme === 'light' ? 'bg-white text-black' : 'bg-black text-white'}`}>
                              <div className="px-5 ">
                                   ...
                                   <p className="text-left m-5 ">{eventData?.description}
                                   </p>
                              </div>
                         </div>
                         <div className={`min-w-[300px] m-5 ${themeData.theme === 'light' ? 'bg-gray-100 text-black' : 'bg-gray-800 text-white'}`}>
                              ...
                              <div className="mb-3 bg-red-100 flex rounded-lg ">
                                   ...
                                   <p className={`text-lg font-bold mx-5 ${themeData.theme === 'light' ? 'text-black' : 'text-black'}`}>{eventData?.state}</p>
                              </div>
                              ...
                              <div className={`p-5 mb-5 border-solid border-2 shadow-md w-auto rounded-lg flex items-start ${themeData.theme === 'light' ? 'bg-white text-black' : 'bg-black text-white'}`}>
                                   ...
                                   <div className='justify-items-start mx-2'>
                                        <p>{eventData?.locName}</p>
                                        <p>{eventData?.locAddress}</p>
                                   </div>
                              </div>
