						...
						<tbody className="">
							{events.map((event, key) => {
								if (totalPages >= 1 && key >= (currentPage - 1) * eventsPerPage && key < currentPage * eventsPerPage) {
									...
									return (
										<tr key={key} className={`border-b-2 p-2 border-2 border-gray-50 bg-gray-100 my-3 transition-colors rounded ${data.theme === 'light' ? 'hover:bg-gray-200' : 'hover:bg-gray-800'}`} onClick={handleClick}>
											<td>
												<img className="p-3 w-[100px] md:w-[200px]" src={require('../assets/Images/emplo.jpg').default.src} height="" alt="" />
											</td>
											<td className="text-black text-md">{event.name.length > 15 ? `${event.name.substring(0, 20)}...` : event.name}</td>
